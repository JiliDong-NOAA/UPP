...
!         CALL CALRH_GSD(P1,T1,Q1,RH)
     ...
     SUBROUTINE CALRH_MPAS(P1,T1,Q1,RH)
      ...
      integer :: j, i
      real :: tx, qvs, qtemp
      real, dimension(ista:iend,jsta:jend) :: P1, T1, Q1, RH
...
! - compute relative humidity
            Tx=T1(I,J)-273.16
            qvs = rslf(P1(I,J)/100.0,T1(I,J))
            if(Tx .le. 0.0) qvs = rsif(P1(I,J)/100.0,T1(I,J))
            qtemp=Q1(I,J)/(1-Q1(I,J)) ! convert to mixing ratio
            RH(I,J) = qtemp / qvs
            RH(I,J) = RH(I,J) * 100.
          ELSE
            RH(I,J) = spval
...
!
      REAL(KIND=8) FUNCTION RSLF(P,T)

      IMPLICIT NONE
      REAL(KIND=4), INTENT(IN):: P, T
      REAL(KIND=8):: ESL,X
      REAL(KIND=8), PARAMETER:: C0= .611583699E03
      REAL(KIND=8), PARAMETER:: C1= .444606896E02
      REAL(KIND=8), PARAMETER:: C2= .143177157E01
      REAL(KIND=8), PARAMETER:: C3= .264224321E-1
      REAL(KIND=8), PARAMETER:: C4= .299291081E-3
      REAL(KIND=8), PARAMETER:: C5= .203154182E-5
      REAL(KIND=8), PARAMETER:: C6= .702620698E-8
      REAL(KIND=8), PARAMETER:: C7= .379534310E-11
      REAL(KIND=8), PARAMETER:: C8=-.321582393E-13
...
!
      REAL(KIND=8) FUNCTION RSIF(P,T)

      IMPLICIT NONE
      REAL(KIND=4), INTENT(IN):: P, T
      REAL(KIND=8):: ESI,X
      REAL(KIND=8), PARAMETER:: C0= .609868993E03
      REAL(KIND=8), PARAMETER:: C1= .499320233E02
      REAL(KIND=8), PARAMETER:: C2= .184672631E01
      REAL(KIND=8), PARAMETER:: C3= .402737184E-1
      REAL(KIND=8), PARAMETER:: C4= .565392987E-3
      REAL(KIND=8), PARAMETER:: C5= .521693933E-5
      REAL(KIND=8), PARAMETER:: C6= .307839583E-7
      REAL(KIND=8), PARAMETER:: C7= .105785160E-9
      REAL(KIND=8), PARAMETER:: C8= .161444444E-12
